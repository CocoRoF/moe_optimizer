...
ARGS = [a for a in ARGS if a != "--no-renorm"]
def _engine_cls(cfg):
    from moe_optimizer.runtime.stream import StreamingOLMoE, StreamingQwen3MoE
    if cfg.get("model_type", "").startswith("qwen"):
        return (lambda *a, **k: StreamingQwen3MoE(*a, renorm=False, **k)) if NO_RENORM else StreamingQwen3MoE
...
for t in TARGETS:
    ...
    renorm = bool(rm.config.get("norm_topk_prob", False)) and not NO_RENORM
...
json.dump(rows, open(f"runs/policy_sweep_{SHORT}{'_norenorm' if NO_RENORM else ''}.json", "w"), indent=1)
print("DONE")
